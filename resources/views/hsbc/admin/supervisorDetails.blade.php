                ...
                <div class="py-12">
                    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8" >
                        ...
                        <div class="p-6 bg-white border-b border-gray-200">
                            ...
                            @if ($webAgents == null)
                            <p class="fs-5"><b class="mr-3"> Team:</b></p>
                            <p><b>No Team Members</b></p>
                            @endif

                            @if (!$webAgents == null)
                            <p class="fs-5 mb-2 "><b class="mr-3">Team:</b> {{$user->supervises->name}}</p>
                            <p class="fs-5 mb-0"><b class="mr-3">{{$user->supervises->name}}'s Members:</b></p>
                            <ul class="pt-0 pl-0 pb-3">
                                @foreach ($webAgents as $webAgent)
                                    <li><a class=" fs-5" href="{{ route('admin.webAgentDetails', ['user' => $webAgent]) }}">{{ $webAgent->name }}</a></li>
                                @endforeach
                            </ul>
                            <p></p>
                            @endif
                            ...
                            @if (Auth::user()->role == 'Admin')
                                    <form action="{{ route('admin.editSupervisor', ['user' => $user]) }}" method="post">
                                        @csrf
                                        @method('GET')
                                        <button style="width: 400px;" class="mt-2 mb-2 d-grid text-white btn border-0 btn-dark"  type='sumbit'>Edit</button>
                                    </form>
                                    <form action="{{ route('admin.destroySupervisor', ['user' => $user]) }}" method="post">
                                        @csrf
                                        @method('DELETE')
                                        <button style="width: 400px;" class="mb-3 mt-2 d-grid text-white btn border-0 btn-danger" type='sumbit'>Delete</button>
                                    </form>
                            @endif
                            <a class="fs-5 mt-2 pt-3" href="{{route('admin.showSupervisors')}}"> < Back </a>
                        </div>
                    </div>
                </div>
            </div>
